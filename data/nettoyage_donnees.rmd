Ce script a pour objectif de nettoyer et de structurer les données d’accidents de la route issues de l’ONISR pour l’année 2021. Il enrichit les données par la création d’un score de sévérité, la classification des accidents selon le moment de la journée et le regroupement des départements en régions, afin d’obtenir une base de données exploitable pour l’analyse statistique.
...
```{r}
write.csv(df_final, "ONISR_2021_donnees_traitees.csv", row.names = FALSE)
file = read.csv("ONISR_2021_donnees_traitees.csv")
length(colnames(file))
colnames(file)
