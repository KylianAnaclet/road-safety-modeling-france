...
# 1. D'abord, on standardise les résidus (pour avoir variance ~ 1)
# 2. On trace l'histogramme en DENSITÉ
hist(resid(modele), 
     breaks = 50, 
     col = "lightblue", 
     freq = FALSE,      # <--- INDISPENSABLE : Passe de l'échelle 15000 à 0.4
     main = "Distribution des Résidus Standardisés",
     xlab = "Résidus standardisés",
     ylim = c(0, 0.5))  # On laisse un peu de marge en haut

# 3. Maintenant la courbe rouge sera visible
...
```{r}
#On constate que les résiduis vérfient bel et bien une loi normale centrée réduite

```

```{r}
modele_complexe <- lm(Score_Severite ~ vma + col + agg*catr + plan, data =df_final)
summary(modele_complexe)
#test sur un autre modèle
```
...
print(vif_values)
#step à utiliser
#Utilsier qu'une partie de la base
```

```{r}

# 3. Création du Modèle OPTIMISÉ (Sans les variables redondantes)
# On enlève surf, nb_voiture, et peut-être des variables "bruit" comme 'Region' ou 'Moment'
#car ce sont des variables redondantes qui expliquent 2 fois la même chose 
modele_optimise <- lm(Score_Severite ~ . - surf - nb_voiture - Region - Moment, data = df_final)
# On a que atm représente la même chose 
# 4. COMPARAISON DES R²
...
```{r}
#A présent, on va extraire les coefficients 
p_values <- summary(modele_optimise)$coefficients[, "Pr(>|t|)"]
# On enlève l'Intercept
p_values <- p_values[names(p_values) != "(Intercept)"]
# 2. TRANSFORMATION (Indispensable pour le graphique)
# On prend le -log10 pour que : Petite p-valeur = Grande barre
# Exemple : p=0.01 devient 2, p=0.0001 devient 4
importance <- -log10(p_values)

# 3. On trie pour avoir les plus forts (ceux avec le plus grand -log10) en premier/haut
importance <- sort(importance, decreasing = FALSE)

# 4. Le Graphe
par(mar=c(5, 12, 4, 2)) # Marge à gauche pour les noms longs
barplot(importance, 
        horiz = TRUE, 
        las = 1, 
        col = "cyan",
        main = "Importance des Facteurs p/r à la p-valeur",
        xlab = "Significativité : Plus c'est grand, plus c'est important")

#Voici toute la démarche qu'on va effectuer : 1) on transforme df => df_final pour avoir uniquement des données numériques et qu'on peut traiter proprement
#2) On vérifie les propriétés du modèle linéaire (déjà vis-à-vis des résidus pour s'assurer que les données qu'on traite a du sens ) + 4 graphes (à expliquer ??)
#3) On utilise le VIF pour déduire les paramètres "redondants" => On en déduit un modèle de regréssion linéaire optimisé 
# Là il faut comprendre cmt cela marche VIF (??)
#4) A présent, on a notre modèle on s'intéresse mtn aux facteurs / coefficients devant chacun des paramètres de notre modèle de régression linéaire => On les trie par la P valeur 
```
