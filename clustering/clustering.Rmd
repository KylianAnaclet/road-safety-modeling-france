...
*Remarque : les variables catégorielles (facteurs) ne sont pas standardisées.* Évaluation de la tendance à former des clusters :

## C. Verification de la légitimité de clustering
...
Le graphique de la méthode du coude montre que la courbe se stabilise à k = 6, tandis que la silhouette présente deux maxima à k = 2 et k = 3.
...
res <- kproto(df_scaled, k = 3, verbose = FALSE)
...
L'analyse des clusters montre que les quatres groupes identifiés présentent des tailles différentes. Le cluster 2 regroupe la majorité des observations, tandis que le cluster 1 correspond a un petit sous-groupe spécifique. Les clusters 3 occupe une taille intermédiaire.
...
Bien que les indicateurs statistiques reflètent la grande complexité des données d'accidentologie, ils confirment la **cohérence globale** de la partition. Le score de silhouette positif indique que les observations sont, en moyenne, bien classées malgré la nature continue du phénomène accidentel qui rend les frontières entre groupes naturellement diffuses.

Dans le détail, le **Cluster 3** se détache avec la meilleure stabilité, validant l'identification d'un profil "routier" spécifique.
